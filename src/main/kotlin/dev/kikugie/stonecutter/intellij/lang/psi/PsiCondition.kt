...
class PsiCondition(node: ASTNode) : ANTLRPsiNode(node), PsiComponent {
    ...
    private fun determineType(): Type {
        val closer = firstChild?.takeIf { it.antlrType == StitcherParser.SCOPE_CLOSE }
    ...
    private fun matchType(closer: Boolean, opener: Int): Type = when(opener) {
        StitcherParser.SCOPE_OPEN -> if (closer) Type.SCOPED_EXTENSION else Type.SCOPED_OPENER
        StitcherParser.SCOPE_WORD -> if (closer) Type.WORD_EXTENSION else Type.WORD_OPENER
